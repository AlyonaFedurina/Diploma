...
t2, x0_2 = rk4(RightPart_u, t0_2, tf_2, x0[:, -1], n, 0)
...
C2_req = np.array([0, 0, 0, 100, 0, 0])
C3_req = np.array([0, 0, 0, 200, 0, 0])
C4_req = np.array([0, 0, 0, 300, 0, 0])
C5_req = np.array([0, 0, 0, 400, 0, 0])
...
t1, x1, X1, Ac1 = Global_rk4(N, RightPart_u, t0, tf, x0_arr, X0_arr, x0, n, k, B_req)
t2, x2, X2, Ac2 = Regrouping(N, RightPart_u, t0_2, tf_2, x1[:, :, -1], X1[:, :, -1], x0_2, n, k, satX_num)
t_full = np.hstack([t1, t2])
X_full = np.concatenate([X1, X2], axis=2)
Ac_full = np.concatenate([Ac1, Ac2], axis=2)
print(x1[:, :, -1])

print(np.shape(X2))
print(np.shape(X_full))
print(np.shape(t_full))


show_B(t_full, Ac_full, N)
show_trajectories_LVLH(t1, X1, N)
show_trajectories_LVLH(t2, X2, N)
# show_trajectories_LVLH(t_full, X_full, N)
# show_Lyapunov(t, Ac1, N)
# show_control_(t, Ac1, N)
