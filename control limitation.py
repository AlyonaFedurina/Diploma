...
def show_B(t, Ac, N):
    ...
    for j in range(4):
        ...
        plt.axvline(x=t[-1], color='g', linestyle='--')
...
def calculate_B4_req(N, a0, x0, B, satX_num, a_req, B4x):
    ...
    B4_req_right_neighbour = a_inB05 + B4x
    if (satX_num == N-1):
...
def Global_rk4(N, fnc, t0, tf, x0_arr, X0_arr, x0, n, k, B_req):
    t = np.linspace(t0, tf, n)
    h = t[1] - t[0]
...
def Regrouping(N, fnc, t0, tf, x_, X_, x0, n, k, satX_num):
    t = np.linspace(t0, tf, n)
    h = t[1] - t[0]
    ...
    for i in range(1, n):
        ...
        t_convergence = 2*n
        for j in range(N):
            if (Ac[j, 6, i] - B4_req[j] < 1e-10 and Ac[j, 4, i] < 1e-6):
                true += 1
        if (true == N):
            t_convergence = i
            break
...
a0 = norm(x0_0[:3])
n1 = 10000
n2 = int((tf_2 - t0_2) / (tf/n1))
i = pi/180 * 0.0
u = 0
o = 0
N = 5
satX_num = 3#int(random.choice(np.linspace(0, N-1, N)))
print(satX_num + 1)
a_req = 150
u_max = 0.005 * 0.80
...
t1, x1, X1, Ac1 = Global_rk4(N, RightPart_u, t0, tf, x0_arr, X0_arr, x0, n1, k, B_req)
print(x1[:, :, -1])
t2, x2, X2, Ac2, t_convergence = Regrouping(N, RightPart_u, t0_2, tf_2, x1[:, :, -1], X1[:, :, -1], x0_2, n2, k, satX_num)
t_full = np.hstack([t1, t2[:t_convergence]])
X_full = np.concatenate([X1, X2[:, :, :t_convergence]], axis=2)
Ac_full = np.concatenate([Ac1, Ac2[:, :, :t_convergence]], axis=2)
# max = 0
# for i in range(n):
#     if (norm(Ac_full[:, :, 7:]) > max):
#         max = norm(Ac_full[:, :, 7:])
# print(max)
print(t_full[-1])

show_B(t_full, Ac_full, N)
#show_trajectories_LVLH(t, X1, N)
show_trajectories_LVLH(t2, X2[:, :, :t_convergence], N)
show_trajectories_LVLH(t_full, X_full, N)
# show_Lyapunov(t, Ac1, N)
show_control_(t_full, Ac_full, N)
